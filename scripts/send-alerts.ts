...
async function main() {
  try {
    ...
    const unifiedActivities = activities.map(a => ({
    ...
    upcoming.slice(0, 10).forEach(item => {
      ...
      if (displayStatus === 'Pendiente' && !isPast) {
      ...
      htmlContent += `
        <div style="background-color: #f8fafc; border-left: 4px solid ${color}; padding: 15px; margin-bottom: 15px; border-radius: 4px;">
          ...
          <div style="font-size: 13px; color: #64748b; margin-bottom: 10px;">
            ...
            <strong>📊 Estatus actual:</strong> ${displayStatus === 'En Proceso' ? '<span style="color: #3b82f6; font-weight: bold;">En Proceso</span>' : displayStatus}
          </div>
