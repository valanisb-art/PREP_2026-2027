...
function Tooltip({
  ...
  const estatusColor = ESTATUS_COLORES[item.estatusRelacion] ?? "#888888";
      ...
      <div className="bg-popover/95 backdrop-blur-xl border border-border/60 rounded-xl shadow-2xl p-4 max-w-xs">
        <div className="flex items-center gap-2 mb-2">
          <span
            ...
            style={{ background: TEMAS_COLORES[item.tema] ?? "#888888" }}
          />
        ...
        {item.observaciones && !/no se identific/i.test(item.observaciones) && (
...
function GanttBarRow({
  ...
  const temaColor = TEMAS_COLORES[item.tema] ?? "#888888";
  const estatusColor = ESTATUS_COLORES[item.estatusRelacion] ?? "#888888";
...
export default function Prep54Page() {
  ...
  // Overrides de fechas importadas (date_overrides_54) que se sobreponen al Gantt.
  const [dateOv54, setDateOv54] = useState<Record<number, { inicio?: string; fin?: string }>>({});
  useEffect(() => {
    supabase
      .from("date_overrides_54")
      .select("no, inicio, fin")
      .then(({ data }: { data: Array<{ no: number; inicio: string | null; fin: string | null }> | null }) => {
        if (data) {
          const m: Record<number, { inicio?: string; fin?: string }> = {};
          data.forEach((d) => { m[d.no] = { inicio: d.inicio ?? undefined, fin: d.fin ?? undefined }; });
          setDateOv54(m);
        }
      });
  }, []);
  const entregablesData = useMemo(
    () =>
      entregables54Gantt.map((e) => {
        const ov = dateOv54[e.no];
        if (!ov) return e;
        return { ...e, inicio: ov.inicio || e.inicio, fin: ov.fin || e.fin };
      }),
    [dateOv54],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return entregablesData.filter((e) => {
  ...
  }, [temaFilter, respFilter, relFilter, searchQuery, entregablesData]);
      ...
      <div className="space-y-6 animate-in fade-in duration-500">
        ...
        >
          ...
          <div className="flex border-b border-border/60">
            <div
              className="shrink-0 bg-card px-3 sm:px-4 flex items-center gap-2 border-r border-border/40 z-10 transition-all duration-150"
              style={{
                width: labelWidth,
                minWidth: labelWidth,
                height: headerHeight,
            ...
            >
              <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider truncate flex-1">
                Entregable
              </span>
              <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider shrink-0 max-w-[90px] text-right hidden md:inline">
                Estatus
              </span>
            </div>
