...
export default function Dashboard() {
  ...
  const computedStats = useMemo(() => {
    ...
    unifiedActivities.forEach(a => {
      ...
      else if (a.status === 'Entregado') entregado++;
    });
    return { total, pendiente, enProceso, entregado };
  }, [unifiedActivities]);

  const alerts = useMemo(() => {
    const list: Array<{ type: 'warning' | 'danger', message: string, activity: any, daysInfo: number }> = [];
    ...
    unifiedActivities.forEach(act => {
      if (act.status !== 'Entregado') {
        if (act.termino) {
          const tDate = new Date(act.termino + "T00:00:00");
          const diffDays = Math.ceil((tDate.getTime() - today.getTime()) / (1000 * 3600 * 24));
          if (diffDays < 0) list.push({ type: 'danger', message: `Actividad atrasada`, activity: act, daysInfo: Math.abs(diffDays) });
          else if (diffDays <= 7) list.push({ type: 'warning', message: `Actividad vence pronto`, activity: act, daysInfo: diffDays });
        }
        
        act.subActivities.forEach(sub => {
          if (sub.termino) {
            const tDate = new Date(sub.termino + "T00:00:00");
            const diffDays = Math.ceil((tDate.getTime() - today.getTime()) / (1000 * 3600 * 24));
            if (diffDays < 0) {
              list.push({ type: 'danger', message: `Subtarea atrasada`, activity: { ...sub, areaResponsable: act.areaResponsable, id: act.id }, daysInfo: Math.abs(diffDays) });
            } else if (diffDays <= 7 && diffDays >= 0) {
              list.push({ type: 'warning', message: `Subtarea vence pronto`, activity: { ...sub, areaResponsable: act.areaResponsable, id: act.id }, daysInfo: diffDays });
            }
          }
        });
      }
