...
import { Search, ChevronDown, ChevronRight, FileText, Clock, PlayCircle, CheckCircle2, CalendarDays } from "lucide-react";
...
export default function HistoricoPage() {
  ...
  // Periodo/calendario seleccionado. Vacío = nada seleccionado (vista en blanco).
  const [selectedCalendar, setSelectedCalendar] = useState<string>("");
  // Vista dentro del periodo: tabla de actividades o reporte con gráficas.
  const [histView, setHistView] = useState<"actividades" | "reporte">("actividades");
  ...
  // Solo se muestra la información cuando hay un periodo seleccionado.
  const showData = selectedCalendar === "prep-2023-2024";
      ...
      <div className="space-y-5">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Consulte los periodos electorales anteriores. Seleccione un periodo para ver sus actividades.</p>
          </div>
          <div className="w-full sm:w-64">
            <Select value={selectedCalendar} onValueChange={setSelectedCalendar}>
              ...
              <SelectContent>
                <SelectItem value="prep-2023-2024">PREP 2023 - 2024</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>

        {/* Vista en blanco hasta seleccionar un periodo */}
        {!showData ? (
          <div className="bg-card rounded-lg shadow-card border border-border/50 px-4 py-16 text-center">
            <CalendarDays className="w-10 h-10 text-muted-foreground/40 mx-auto mb-3" />
            <p className="text-sm text-muted-foreground">Seleccione un periodo para ver sus actividades.</p>
          </div>
        ) : (
        <>
        {/* Interruptor de vista: Actividades (tabla) / Reporte (gráficas) */}
        <div className="flex gap-1.5">
          {(["actividades", "reporte"] as const).map((v) => (
            <button
              key={v}
              onClick={() => setHistView(v)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${histView === v ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
            >
              {v === "actividades" ? "Actividades" : "Reporte"}
            </button>
          ))}
        </div>
