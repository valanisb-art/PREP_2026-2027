...
export interface UnifiedSubActivity {
  ...
  status: ActivityStatus;
  avance: number;
  checked: boolean; // subactividad marcada como revisada/entregada
}

// Reexportado desde la lógica pura (src/lib/alerts.ts) para poder probarla aislada.
export { computeEntregableProgress } from "@/lib/alerts";
  ...
  const fetchData = useCallback(async () => {
    ...
    const prepStatic = activities.filter(a => {
      ...
      staticSubData.forEach(sub => {
        ...
        let raw = ov?.actividad ?? sub.actividad;
        let checked = false;
        if (typeof raw === 'string' && raw.includes('|||CHECKED:true')) { checked = true; raw = raw.replace('|||CHECKED:true', ''); }
        const { text, inicio: parsedInicio, termino: parsedTermino } = parseDates(raw);
        ...
        mappedSubs.push({
          ...
          checked,
          status: checked ? 'Entregado' : calculateStatus(finalInicio, finalTermino),
          avance: checked ? 100 : calculateProgress(finalInicio, finalTermino, 'Pendiente')
        });
      });

      // Add custom subs for this static activity
      (customSubs || []).filter(cs => cs.parent_static_activity_id === a.id).forEach(cs => {
        let rawCs = cs.actividad;
        let checkedCs = false;
        if (typeof rawCs === 'string' && rawCs.includes('|||CHECKED:true')) { checkedCs = true; rawCs = rawCs.replace('|||CHECKED:true', ''); }
        const { text, inicio, termino } = parseDates(rawCs);
        mappedSubs.push({
          id: cs.id,
          customId: cs.id,
          isCustom: true,
          ...
          termino,
          checked: checkedCs,
          status: checkedCs ? 'Entregado' : calculateStatus(inicio, termino),
          avance: checkedCs ? 100 : calculateProgress(inicio, termino, 'Pendiente')
        });
      });
